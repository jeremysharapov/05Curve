...
void add_curve( struct matrix *points, 
  ...
  double t, x, y, xone, yone;
  x = x0;
  y = y0;

  for (t = step; t < 1.01; t += step){
    xone = (xco -> m[0][0] * pow(t, 3)) + (xco -> m[1][0] * pow(t, 2)) + (xco -> m[2][0] * t) + (xco -> m[3][0]);
    yone = (yco -> m[0][0] * pow(t, 3)) + (yco -> m[1][0] * pow(t, 2)) + (yco -> m[2][0] * t) + (yco -> m[3][0]);
    add_edge(points, x, y, 0, xone, yone, 0);
    x = xone;
    y = yone;
  }
  free_matrix(xco);
